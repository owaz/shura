  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
      ...
      {source && <PortalDialog title="Crop your photo" description="Position your face in the circular preview and adjust the zoom." onClose={() => { URL.revokeObjectURL(source); setSource(''); setFile(null); }} size="max-w-md" closeDisabled={uploading} initialFocusSelector="#photoZoom">
          ...
          <div className="mt-6 flex justify-end gap-3"><button type="button" disabled={uploading} onClick={() => { URL.revokeObjectURL(source); setSource(''); setFile(null); }} className="rounded-full px-4 py-2.5 font-semibold text-brown-soft hover:bg-sand disabled:cursor-not-allowed disabled:opacity-60">Cancel</button><button type="button" disabled={uploading} onClick={() => void cropAndUpload()} className="rounded-full bg-[#8C4F3A] px-5 py-2.5 font-semibold text-white disabled:opacity-60">{uploading ? 'Uploading…' : 'Use this photo'}</button></div>
      </PortalDialog>}
    </div>
  );
};
