...
- Client: legacy payment plus `/portal/*` home, onboarding, booking, sessions, therapist, profile, preferences, notifications, and billing.
...
- `/portal/billing` presents the server-derived paid, covered, free, or disabled state, upcoming-session payment status, paginated payment/refund history, and authenticated PDF receipt downloads. Billing navigation follows `client_portal_features.billingEnabled`. The UI does not claim saved-card support because the current Razorpay contract is one-time checkout at booking.
